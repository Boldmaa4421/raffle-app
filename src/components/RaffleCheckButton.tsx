...
export default function RaffleCheckButton({
  ...
  // ✅ iOS: popup нээгдэхэд input focus өгнө + body scroll lock
  useEffect(() => {
    if (!open) return;
    ...
    // iOS дээр найдвартай: RAF + жижиг timeout
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[100]">
          {/* overlay (арын card-аас ялгаруулна) */}
          <div
          ...
          <div className="absolute inset-0 flex items-center justify-center p-4">
            {/* modal */}
            <div
              className="relative w-full max-w-2xl rounded-2xl border border-white/15 bg-black/85 shadow-2xl overflow-hidden
            ...
            >
              ...
              {/* body scroll */}
              <div className="p-5 overflow-y-auto [-webkit-overflow-scrolling:touch]">
                {/* input row */}
                <div className="flex flex-col sm:flex-row gap-2">
                  <input
                    ...
                    // ✅ iPhone дээр + оруулах боломжтой + keypad гарах
                    inputMode="tel"
                    type="tel"
                    autoComplete="tel"
                    enterKeyHint="search"
                    className="flex-1 rounded-xl px-4 py-3 bg-white/5 border border-white/10
                      outline-none text-white text-[16px]"
                  />
                              ...
                              {g.codes.map((c) => (
                                <span
                                  ...
                                  className="rounded-full border border-emerald-500/25 bg-emerald-500/10
                                    px-2.5 py-1 text-xs font-extrabold text-emerald-200 text-center"
                                >
                                  {prettyCode(c)}
                                </span>
                              ))}
              ...
              <div className="h-[max(env(safe-area-inset-bottom),12px)] bg-black/85" />
            </div>
          </div>
        </div>
      )}
